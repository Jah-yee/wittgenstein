# Wittgenstein / Modality Harness — Build Book

## 0. Executive position

### The project in one sentence

**Wittgenstein is a modality harness / multimodal adapter that lets a text-first LLM output real files — PNG, WAV/MP3, MP4, JSON sensor traces — by shifting modality-specific work into a portable middleware layer instead of requiring heavyweight model-side multimodal training.**
...
- **Wittgenstein** — product/project name
...
> Wittgenstein is a multimodal compiler layer that lets text-first LLMs emit real media files through portable codecs, local renderers, and lightweight adapter bundles.
...
```json
{
  ...
  "elements": [
    { "type": "title", "text": "WITTGENSTEIN", "x": 72, "y": 120, "size": 96 },
...
- your own unified “wittgenstein video” experience.
...
- `@wittgenstein/codec-image-neural`
- `@wittgenstein/codec-audio-library`
...
```bash
wittgenstein init
wittgenstein image "editorial poster about multimodal harnesses" --out out.png
wittgenstein audio "calm Chinese voice reading the release note" --out out.mp3
wittgenstein video "10-second product teaser" --out out.mp4
wittgenstein sensor "simulate a 12-second ECG trace with mild arrhythmia" --out ecg.json
```
...
**Wittgenstein: a modality harness for text-first models**
