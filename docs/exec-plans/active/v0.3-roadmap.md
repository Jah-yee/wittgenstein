...
- Hybrid Image Code replaces `scene-spec JSON as terminal image IR`
- `Semantic IR` remains canonical but shifts to model-side organization / user-facing inspection
...
- `one-shot hybrid` is the standard lane; `two-pass compile` is the high-quality lane
