...
| P2  | Procedural + chaotic operators                | ✅ within a pinned numerical backend | ❌ no             | ✅ compatible                   | **Brief L §S1 — adopt at v0.4 post-M3**                    |
...
A second-order or third-order ODE system integrated with a fixed-step, fixed-precision integrator (RK4 with fixed step size and fixed `f64` precision) is **mathematically chaotic** (positive Lyapunov exponent, sensitive dependence on initial conditions) **AND replayable within a pinned numerical backend** (same seed + same integrator step + same backend discipline → identical trajectory bytes). Cross-platform byte identity is a separate audit item, not an assumption. The relevant systems for sensor-shaped signals:
...
**Claim.** Add Lorenz / Rössler / Chua / Hopfield-oscillator / intermittency map as new entries in the sensor operator library. Each integrated with fixed-step RK4 and fixed `f64` precision so trajectories are deterministic within a pinned numerical backend and golden-testable. The LLM's `SensorSpec` gains the ability to compose chaotic operators alongside the existing periodic / decay / noise primitives.

- **Supporting:** Real biological signals exhibit deterministic chaos signatures (positive `λ_max` documented for ECG, EEG, HRV per [PubMed 8373884](https://pubmed.ncbi.nlm.nih.gov/8373884/), [PubMed 12779752](https://pubmed.ncbi.nlm.nih.gov/12779752/), [PLOS One 2014](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0105622)). Procedural sum-of-sines does not. Closing this gap closes a known weakness of synthetic biomedical-style data without breaking determinism inside a pinned backend. Cross-platform byte identity is conditional on the integrator-and-precision contract below: avoid or explicitly pin transcendental calls, disable FMA contraction where relevant, audit auto-vectorization, and pin the JS engine / libm / WASM backend used for goldens. The operator surface is small (~5 new operators); the work is bounded.
...
4. **The integrator-and-precision contract is mechanical, not promissory.** Any implementation must name the exact backend contract that produces byte replay: basic IEEE-754 operations only unless each transcendental call site is documented and pinned; FMA contraction off where a compiler can fuse `a*b+c`; auto-vectorization audited; JS engine / libm / WASM backend pinned for the TS path. Tests assert byte-identical replay across three back-to-back runs on the pinned backend, and cross-platform promotion requires the paired audit to pass.
...
| v0.4 (paired)  | Integrator + precision audit (backend contract + cross-platform RK4 byte identity)                         | `packages/codec-sensor/test/integrator-determinism.test.ts`       | folded into operator-extension issue |
