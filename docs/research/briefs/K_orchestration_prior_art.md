...
## Primitive mapping — where each external primitive lives in Wittgenstein today

The following table is the explicit integration into our existing harness + HCI surfaces. Each external primitive maps to a current file or surface, with the gap (if any) flagged. This is the "embed into our work" step the brief is meant to enable.

| External primitive                                             | Source  | Wittgenstein equivalent (today)                                                                                                               | Gap                                                                                               | Action in v0.3                                                                                                  |
| -------------------------------------------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `WORKFLOW.md` (Symphony)                                       | S1      | _(none)_                                                                                                                                      | Whole primitive missing                                                                           | **Add** at repo root (new — handoff brief at `docs/handoff/workflow-md-spec.md`)                                |
| State machine (`Unclaimed`→`Released`)                         | S1      | Implicit in GitHub Issue states + label set per `docs/labels.md`                                                                              | No `Claimed`/`Running` runtime states; we don't dispatch agents per issue                         | Encoded as agent-eligible-labels + GitHub state set in `WORKFLOW.md` §1                                         |
| Per-issue isolated workspace                                   | S1      | Worktree pattern in `docs/engineering-discipline.md` (we use `git worktree` for parallel branches)                                            | Convention not codified for agent dispatch                                                        | Codified in `WORKFLOW.md` §3 (workspace root + sanitized name pattern)                                          |
| Prompt template auto-loads project standards                   | S1 + S2 | `PROMPT.md` (the public agent prompt) + `AGENTS.md` (depth) + `docs/engineering-discipline.md`                                                | No template variable system; no per-task injection of relevant handoff brief                      | `WORKFLOW.md` §4 names the template variables (`issue`, `labels`, matching `docs/handoff/<slice>.md` if exists) |
| Reconciliation tick + retry policy                             | S1      | _(none — we are 0 open PRs at sprint end; manual)_                                                                                            | At our scale, not needed in code; codified as **policy** in `WORKFLOW.md` for future orchestrator | `WORKFLOW.md` §5                                                                                                |
| Snapshot/monitoring API                                        | S1      | _(none)_                                                                                                                                      | Not needed at v0.3; v0.4 if orchestrator lands                                                    | Out of scope                                                                                                    |
| **Spec auto-injection** (Trellis)                              | S2      | `PROMPT.md` + `docs/engineering-discipline.md` + `.github/workflows/doctrine-guardrail.yml`                                                   | Already covered in 3 places                                                                       | No-op (K.4 verdict)                                                                                             |
| **Workspace journal** (Trellis)                                | S2      | `artifacts/runs/<id>/manifest.json` (per-run; not cross-run)                                                                                  | Cross-run continuity                                                                              | v0.4 (`parentRunId` + `priorRunNotes` fields in manifest schema)                                                |
| **Sub-agent role specialization** (Trellis)                    | S2      | Two-hat review (researcher / hacker) per ADR-0013                                                                                             | Pre-execution role split (planner / generator / evaluator)                                        | Defer (K.3 verdict; M2 Slice E #118 is test case)                                                               |
| **Hook** (Trellis)                                             | S2      | `.github/workflows/*.yml` (auto-labeler, doctrine-guardrail, link-check, codec-cruiser)                                                       | Already covered                                                                                   | No-op                                                                                                           |
| **Three-agent pattern** (Anthropic harness)                    | S3      | Two-hat review (post-hoc); handoff briefs are planner outputs; PRs are generator outputs                                                      | Evaluator-as-distinct-agent missing                                                               | Defer to v0.4 (K.3); M2 Slice E test case                                                                       |
| **Sprint contracts** (Anthropic harness)                       | S3      | `docs/handoff/<slice>.md` "Done when" sections; Brief J / I `## Verdict` sections                                                             | Already covered, well                                                                             | No-op                                                                                                           |
| **Concrete grading thresholds** (Anthropic)                    | S3      | Brief J §Verdict 4 fixture-class verdicts; CI-gated parity tests; sensor goldens                                                              | Already covered                                                                                   | No-op                                                                                                           |
| **Communication via files** (Anthropic)                        | S3      | Handoff briefs + manifest spine + brief-to-RFC-to-ADR chain                                                                                   | Already covered                                                                                   | No-op                                                                                                           |
| **Brain / Hands / Session split** (Anthropic)                  | S4      | **Brain** = harness in `packages/core/src/runtime/`; **Hands** = `packages/codec-*` + `packages/sandbox` (stub); **Session** = manifest spine | Implicit; not labeled in docs                                                                     | Reframe in `docs/reproducibility.md` addendum (K.1 verdict) — make the OS-style split explicit                  |
| **Stateless harness + `wake(sessionId)`** (Anthropic)          | S4      | Manifest replay-from-disk is the spiritual equivalent; `--manifest <path>` CLI flag exists per RFC-0002                                       | No literal `wake` API                                                                             | Out of scope at our scale; cited as v0.4 horizon                                                                |
| **Credential isolation** (Anthropic)                           | S4      | ADR-0016 boundary for `polyglot-mini` painter sandbox; CLI provider-config separation per ADR-0009                                            | Orchestrator-level credential proxy missing                                                       | v0.4 (depends on MCP adoption decision)                                                                         |
| **Tool interface `execute(name, input) → string`** (Anthropic) | S4      | `Codec<Req, Art>.produce` is the equivalent at the modality layer (RFC-0001 / ADR-0008)                                                       | None                                                                                              | No-op — we already have this shape                                                                              |

**Net:** of 17 mapped primitives, **11 are already covered** in the existing harness + HCI surfaces (renamed or labeled differently). **3 are deferred to v0.4** (workspace journal, three-agent pattern, MCP credential proxy). **3 land in v0.3**: `WORKFLOW.md` (new), `docs/reproducibility.md` addendum (Brain/Hands/Session naming), and the cross-link in `docs/agent-guides/README.md`. The brief's verdicts are concrete because the gap is small.
