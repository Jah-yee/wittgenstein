...
- L2 IR / Codec: one structured modality contract per modality; image now uses a hybrid image-code container rather than scene JSON alone.
...
- Image has exactly one shipping path: `LLM -> structured image-code container -> seed expander / adapter -> frozen decoder -> PNG`.
- Inside that one image path, `Semantic IR` remains canonical for model-side organization and user inspection, but `Visual Seed Token` is the primary image research layer.
...
| **Adapter**   | L4    | Learned bridge / seed expander (hybrid image code → fuller latent tokens); optional, image only today |
...
- **Hybrid Image Code is the image doctrine.** `Semantic IR` remains in the image path, but not as the terminal image research object; `Visual Seed Token` is first-class, `one-shot hybrid` and `two-pass compile` are both legal, and the image adapter is primarily a seed expander / visual-code compiler. Source: [`docs/rfcs/0006-hybrid-image-code.md`](docs/rfcs/0006-hybrid-image-code.md), ratified by [ADR-0018](docs/adrs/0018-hybrid-image-code-and-visual-seed-token.md).
